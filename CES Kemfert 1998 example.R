...
summary( cesBfgs3 )
## L-BFGS-B
cesBfgsCon1 <- cesEst( "Y", xNames1, tName = "time", data = GermanIndustry, method = "L-BFGS-B", control = list( maxit = 5000 ) )
summary( cesBfgsCon1 )
...
cesSannLm3 <- cesEst( "Y", xNames3, tName = "time", data = GermanIndustry,
                      start = coef( cesSann3 ), control = nls.lm.control( maxiter = 1000, maxfev = 2000 ) )
summary( cesSannLm3 )
...
cesLmFixed2 <- cesEst( "Y", xNames2f, data = GermanIndustry,
                       rho1 = 0.2155, rho = 1.1816, control = nls.lm.control( maxiter = 1000, maxfev = 2000 ) )
summary( cesLmFixed2 )
...
print( matrix( c( cesNmFixed3$rss, cesBfgsFixed3$rss, cesLmFixed3$rss, cesNewtonFixed3$rss, cesPortFixed3$rss ), ncol = 1 ), digits = 16 )
...
tabCoef <- array( NA, dim = c( 9, 7, length( metNames ) ), dimnames = list(
...
for( indNo in 1:length( indAbbr ) ) {
  ...
  for( modNo in 1:3 ) {
    ...
    ## PORT
    indRes[[ indNo ]][[ modNo ]]$port <- cesEst( yIndName, xIndNames, tName = "time", data = GermanIndustry, method = "PORT",
                                                 control = list( eval.max = 2000, iter.max = 2000 ) )
